...
import art.arcane.amulet.functional.Supplier3R;
...
public interface Hunk<T> {
    ...
    static <A, B> Hunk<B> convertedReadView(Hunk<A> src, Function.One<A, B> reader) {
        return new FunctionalHunkView<A, B>(src, reader, null);
    }

    static <A, B> Hunk<B> convertedWriteView(Hunk<A> src, Function.One<B, A> writer) {
        return new FunctionalHunkView<A, B>(src, null, writer);
    }

    static <A, B> Hunk<B> convertedReadWriteView(Hunk<A> src, Function.One<A, B> reader, Function.One<B, A> writer) {
     ...
     */
    @SafeVarargs
    static <T> Hunk<T> combined(Function.Three<Integer, Integer, Integer, Hunk<T>> factory, Hunk<T>... hunks) {
        int w = 0;
        int h = 0;
        int d = 0;
    ...
    static <A, B> void computeDual2D(int parallelism, Hunk<A> a, Hunk<B> b, Consume.Five<Integer, Integer, Integer, Hunk<A>, Hunk<B>> v) {
        ...
        List<Runnable> rq = new ArrayList<>(parallelism);
    ...
    static <A, B> void getDualSections2D(int sections, Hunk<A> a, Hunk<B> b, Consume.Six<Integer, Integer, Integer, Hunk<A>, Hunk<B>, Runnable> v, Consume.Five<Integer, Integer, Integer, Hunk<A>, Hunk<B>> inserterAB) {
        if (a.getWidth() != b.getWidth() || a.getHeight() != b.getHeight() || a.getDepth() != b.getDepth()) {
            throw new RuntimeException("Hunk sizes must match!");
        }
    ...
    static <A, B> void getDualSection(int x, int y, int z, int x1, int y1, int z1, Hunk<A> a, Hunk<B> b, Consume.Three<Hunk<A>, Hunk<B>, Runnable> v, Consume.Five<Integer, Integer, Integer, Hunk<A>, Hunk<B>> inserter) {
    ...
    static Vec rotatedBounding(int w, int h, int d, double x, double y, double z) {
        int[] iii = {0, 0, 0};
        int[] aaa = {w, h, d};
        int[] aai = {w, h, 0};
        int[] iaa = {0, h, d};
        ...
        return Vec.of(maxX - minX, maxY - minY, maxZ - minZ);
    }
    ...
    default Hunk<T> listen(Consume.Four<Integer, Integer, Integer, T> l) {
     ...
     * @return this
     */
    default Hunk<T> iterateSurfaces2D(Predicate<T> p, Consume.Eight<Integer, Integer, Integer, Integer, Integer, Integer, Integer, Hunk<T>> c) {
     ...
     * @return this
     */
    default Hunk<T> iterateSurfaces2D(int parallelism, Predicate<T> p, Consume.Eight<Integer, Integer, Integer, Integer, Integer, Integer, Integer, Hunk<T>> c) {
     ...
     * @return this
     */
    default Hunk<T> iterate2DTop(Consume.Five<Integer, Integer, Integer, Integer, Hunk<T>> c) {
     ...
     * @return this
     */
    default Hunk<T> iterate2DTop(int parallelism, Consume.Five<Integer, Integer, Integer, Integer, Hunk<T>> c) {
    ...
    default Hunk<T> iterate(Predicate<T> p, Consume.Three<Integer, Integer, Integer> c) {
        return iterate(getIdeal3DParallelism(), p, c);
    }

    default Hunk<T> iterate(int parallelism, Predicate<T> p, Consume.Three<Integer, Integer, Integer> c) {
    ...
    default Hunk<T> iterate(Predicate<T> p, Consume.Four<Integer, Integer, Integer, T> c) {
        return iterate(getIdeal3DParallelism(), p, c);
    }

    default Hunk<T> iterate(int parallelism, Predicate<T> p, Consume.Four<Integer, Integer, Integer, T> c) {
        iterate(parallelism, (x, y, z, t) ->
        {
            if (p.test(t)) {
                c.accept(x, y, z, t);
    ...
    default Hunk<T> iterate(Consume.Three<Integer, Integer, Integer> c) {
        return iterate(getIdeal3DParallelism(), c);
    }

    default Hunk<T> iterateSync(Consume.Three<Integer, Integer, Integer> c) {
    ...
    default Hunk<T> iterateSync(Consume.Four<Integer, Integer, Integer, T> c) {
        for (int i = 0; i < getWidth(); i++) {
            for (int j = 0; j < getHeight(); j++) {
                for (int k = 0; k < getDepth(); k++) {
    ...
    default Hunk<T> updateSync(Function.Four<Integer, Integer, Integer, T, T> c) {
        for (int i = 0; i < getWidth(); i++) {
            for (int j = 0; j < getHeight(); j++) {
                for (int k = 0; k < getDepth(); k++) {
    ...
    default Hunk<T> iterateSyncIO(Consume.FourIO<Integer, Integer, Integer, T> c) throws IOException {
        for (int i = 0; i < getWidth(); i++) {
            for (int j = 0; j < getHeight(); j++) {
                for (int k = 0; k < getDepth(); k++) {
                    c.accept(i, j, k, get(i, j, k));
    ...
    default Hunk<T> iterate(int parallelism, Consume.Three<Integer, Integer, Integer> c) {
    ...
    default Hunk<T> iterate(Consume.Four<Integer, Integer, Integer, T> c) {
        return iterate(getIdeal3DParallelism(), c);
    }

    default Hunk<T> iterate(int parallelism, Consume.Four<Integer, Integer, Integer, T> c) {
        compute3D(parallelism, (x, y, z, h) ->
        {
            for (int i = 0; i < h.getWidth(); i++) {
                for (int j = 0; j < h.getHeight(); j++) {
    ...
    default Hunk<T> compute2D(Consume.Four<Integer, Integer, Integer, Hunk<T>> v) {
        return compute2D(getIdeal2DParallelism(), v);
    }

    default Hunk<T> compute2D(int parallelism, Consume.Four<Integer, Integer, Integer, Hunk<T>> v) {
        ...
        if (isAtomic()) {
            ...
            List<Runnable> rq = new ArrayList<>();
    ...
    default Hunk<T> compute2DYRange(int parallelism, int ymin, int ymax, Consume.Four<Integer, Integer, Integer, Hunk<T>> v) {
        if (get2DDimension(parallelism) == 1) {
            v.accept(0, 0, 0, this);
            return this;
        }
    ...
    default Hunk<T> compute3D(Consume.Four<Integer, Integer, Integer, Hunk<T>> v) {
        return compute3D(getIdeal3DParallelism(), v);
    }

    default Hunk<T> compute3D(int parallelism, Consume.Four<Integer, Integer, Integer, Hunk<T>> v) {
    ...
    default Hunk<T> getSections2D(int sections, Consume.Five<Integer, Integer, Integer, Hunk<T>, Runnable> v) {
        return getSections2D(sections, v, this::insert);
    }

    default Hunk<T> getSectionsAtomic2D(int sections, Consume.Four<Integer, Integer, Integer, Hunk<T>> v) {
    ...
    default Hunk<T> getSections2D(int sections, Consume.Five<Integer, Integer, Integer, Hunk<T>, Runnable> v, Consume.Four<Integer, Integer, Integer, Hunk<T>> inserter) {
        int dim = get2DDimension(sections);
    ...
    default Hunk<T> getSections2DYLimit(int sections, int ymin, int ymax, Consume.Five<Integer, Integer, Integer, Hunk<T>, Runnable> v, Consume.Four<Integer, Integer, Integer, Hunk<T>> inserter) {
        int dim = get2DDimension(sections);
    ...
    default Hunk<T> getSections3D(int sections, Consume.Five<Integer, Integer, Integer, Hunk<T>, Runnable> v) {
        return getSections3D(sections, v, (xx, yy, zz, c) -> insert(xx, yy, zz, c));
    }

    default Hunk<T> getSections3D(int sections, Consume.Five<Integer, Integer, Integer, Hunk<T>, Runnable> v, Consume.Four<Integer, Integer, Integer, Hunk<T>> inserter) {
    ...
    default Hunk<T> getSection(int x, int y, int z, int x1, int y1, int z1, Consume.Two<Hunk<T>, Runnable> v) {
        return getSection(x, y, z, x1, y1, z1, v, (xx, yy, zz, c) -> insert(xx, yy, zz, c));
    }

    default Hunk<T> getSection(int x, int y, int z, int x1, int y1, int z1, Consume.Two<Hunk<T>, Runnable> v, Consume.Four<Integer, Integer, Integer, Hunk<T>> inserter) {
    ...
    default Vec getCenter() {
        return Vec.of(getCenterX(), getCenterY(), getCenterZ());
    }
